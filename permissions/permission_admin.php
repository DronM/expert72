<?php
/**
	DO NOT MODIFY THIS FILE!	
	Its content is generated automaticaly from template placed at build/permissions/permission_php.tmpl.	
 */
function method_allowed($contrId,$methId){
$permissions = array();

					$permissions['Constant_Controller_set_value']=TRUE;
				
					$permissions['Constant_Controller_get_list']=TRUE;
				
					$permissions['Constant_Controller_get_object']=TRUE;
				
					$permissions['Constant_Controller_get_values']=TRUE;
				
					$permissions['Enum_Controller_get_enum_list']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_insert']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_update']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_delete']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_get_list']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_get_object']=TRUE;
				
					$permissions['MainMenuContent_Controller_insert']=TRUE;
				
					$permissions['MainMenuContent_Controller_update']=TRUE;
				
					$permissions['MainMenuContent_Controller_delete']=TRUE;
				
					$permissions['MainMenuContent_Controller_get_list']=TRUE;
				
					$permissions['MainMenuContent_Controller_get_object']=TRUE;
				
					$permissions['View_Controller_get_list']=TRUE;
				
					$permissions['View_Controller_complete']=TRUE;
				
					$permissions['View_Controller_get_section_list']=TRUE;
				
					$permissions['VariantStorage_Controller_insert']=TRUE;
				
					$permissions['VariantStorage_Controller_upsert_filter_data']=TRUE;
				
					$permissions['VariantStorage_Controller_upsert_col_visib_data']=TRUE;
				
					$permissions['VariantStorage_Controller_upsert_col_order_data']=TRUE;
				
					$permissions['VariantStorage_Controller_update']=TRUE;
				
					$permissions['VariantStorage_Controller_delete']=TRUE;
				
					$permissions['VariantStorage_Controller_get_list']=TRUE;
				
					$permissions['VariantStorage_Controller_get_object']=TRUE;
				
					$permissions['VariantStorage_Controller_get_filter_data']=TRUE;
				
					$permissions['VariantStorage_Controller_get_col_visib_data']=TRUE;
				
					$permissions['VariantStorage_Controller_get_col_order_data']=TRUE;
				
					$permissions['Bank_Controller_get_list']=TRUE;
				
					$permissions['Bank_Controller_get_object']=TRUE;
				
					$permissions['Bank_Controller_complete']=TRUE;
				
					$permissions['About_Controller_get_object']=TRUE;
				
					$permissions['User_Controller_insert']=TRUE;
				
					$permissions['User_Controller_update']=TRUE;
				
					$permissions['User_Controller_delete']=TRUE;
				
					$permissions['User_Controller_get_list']=TRUE;
				
					$permissions['User_Controller_get_object']=TRUE;
				
					$permissions['User_Controller_complete']=TRUE;
				
					$permissions['User_Controller_get_profile']=TRUE;
				
					$permissions['User_Controller_password_recover']=TRUE;
				
					$permissions['User_Controller_register']=TRUE;
				
					$permissions['User_Controller_name_check']=TRUE;
				
					$permissions['User_Controller_login']=TRUE;
				
					$permissions['User_Controller_login_refresh']=TRUE;
				
					$permissions['User_Controller_logout']=TRUE;
				
					$permissions['User_Controller_logout_html']=TRUE;
				
					$permissions['User_Controller_email_confirm']=TRUE;
				
					$permissions['MailForSending_Controller_get_list']=TRUE;
				
					$permissions['TimeZoneLocale_Controller_get_list']=TRUE;
				
					$permissions['TimeZoneLocale_Controller_get_object']=TRUE;
				
					$permissions['EmailTemplate_Controller_update']=TRUE;
				
					$permissions['EmailTemplate_Controller_get_list']=TRUE;
				
					$permissions['EmailTemplate_Controller_get_object']=TRUE;
				
					$permissions['Client_Controller_insert']=TRUE;
				
					$permissions['Client_Controller_update']=TRUE;
				
					$permissions['Client_Controller_delete']=TRUE;
				
					$permissions['Client_Controller_get_list']=TRUE;
				
					$permissions['Client_Controller_get_object']=TRUE;
				
					$permissions['Client_Controller_complete']=TRUE;
				
					$permissions['ClientResponsablePerson_Controller_insert']=TRUE;
				
					$permissions['ClientResponsablePerson_Controller_update']=TRUE;
				
					$permissions['ClientResponsablePerson_Controller_delete']=TRUE;
				
					$permissions['ClientResponsablePerson_Controller_get_list']=TRUE;
				
					$permissions['ClientResponsablePerson_Controller_get_object']=TRUE;
				
					$permissions['ClientBankAccount_Controller_insert']=TRUE;
				
					$permissions['ClientBankAccount_Controller_update']=TRUE;
				
					$permissions['ClientBankAccount_Controller_delete']=TRUE;
				
					$permissions['ClientBankAccount_Controller_get_list']=TRUE;
				
					$permissions['ClientBankAccount_Controller_get_object']=TRUE;
				
					$permissions['ApplicationPdTemplate_Controller_insert']=TRUE;
				
					$permissions['ApplicationPdTemplate_Controller_update']=TRUE;
				
					$permissions['ApplicationPdTemplate_Controller_delete']=TRUE;
				
					$permissions['ApplicationPdTemplate_Controller_get_list']=TRUE;
				
					$permissions['ApplicationPdTemplate_Controller_get_object']=TRUE;
				
					$permissions['ApplicationDostTemplate_Controller_insert']=TRUE;
				
					$permissions['ApplicationDostTemplate_Controller_update']=TRUE;
				
					$permissions['ApplicationDostTemplate_Controller_delete']=TRUE;
				
					$permissions['ApplicationDostTemplate_Controller_get_list']=TRUE;
				
					$permissions['ApplicationDostTemplate_Controller_get_object']=TRUE;
				
					$permissions['ApplicationTemplateContent_Controller_insert']=TRUE;
				
					$permissions['ApplicationTemplateContent_Controller_update']=TRUE;
				
					$permissions['ApplicationTemplateContent_Controller_delete']=TRUE;
				
					$permissions['ApplicationTemplateContent_Controller_get_list']=TRUE;
				
					$permissions['ApplicationTemplateContent_Controller_get_object']=TRUE;
				
					$permissions['ClientSearch_Controller_search']=TRUE;
				
					$permissions['Kladr_Controller_get_region_list']=TRUE;
				
					$permissions['Kladr_Controller_get_raion_list']=TRUE;
				
					$permissions['Kladr_Controller_get_naspunkt_list']=TRUE;
				
					$permissions['Kladr_Controller_get_gorod_list']=TRUE;
				
					$permissions['Kladr_Controller_get_ulitsa_list']=TRUE;
				
					$permissions['Kladr_Controller_get_from_naspunkt']=TRUE;
				
					$permissions['Office_Controller_insert']=TRUE;
				
					$permissions['Office_Controller_update']=TRUE;
				
					$permissions['Office_Controller_delete']=TRUE;
				
					$permissions['Office_Controller_get_list']=TRUE;
				
					$permissions['Office_Controller_get_object']=TRUE;
				
					$permissions['Captcha_Controller_get']=TRUE;
				
					$permissions['ApplicationContractor_Controller_insert']=TRUE;
				
					$permissions['ApplicationContractor_Controller_update']=TRUE;
				
					$permissions['ApplicationContractor_Controller_delete']=TRUE;
				
					$permissions['ApplicationContractor_Controller_get_list']=TRUE;
				
					$permissions['ApplicationContractor_Controller_get_object']=TRUE;
				
					$permissions['TechnicalFeature_Controller_insert']=TRUE;
				
					$permissions['TechnicalFeature_Controller_update']=TRUE;
				
					$permissions['TechnicalFeature_Controller_delete']=TRUE;
				
					$permissions['TechnicalFeature_Controller_get_list']=TRUE;
				
					$permissions['TechnicalFeature_Controller_get_object']=TRUE;
				
					$permissions['DownloadFileType_Controller_insert']=TRUE;
				
					$permissions['DownloadFileType_Controller_update']=TRUE;
				
					$permissions['DownloadFileType_Controller_delete']=TRUE;
				
					$permissions['DownloadFileType_Controller_get_list']=TRUE;
				
					$permissions['DownloadFileType_Controller_get_object']=TRUE;
				
					$permissions['Application_Controller_insert']=TRUE;
				
					$permissions['Application_Controller_update']=TRUE;
				
					$permissions['Application_Controller_delete']=TRUE;
				
					$permissions['Application_Controller_get_object']=TRUE;
				
					$permissions['Application_Controller_get_print']=TRUE;
				
					$permissions['Application_Controller_get_list']=TRUE;
				
					$permissions['Application_Controller_complete']=TRUE;
				
					$permissions['Application_Controller_get_client_list']=TRUE;
				
					$permissions['Application_Controller_remove_file']=TRUE;
				
					$permissions['Application_Controller_get_file']=TRUE;
				
					$permissions['Application_Controller_zip_all']=TRUE;
				
					$permissions['ApplicationClient_Controller_insert']=TRUE;
				
					$permissions['ApplicationClient_Controller_update']=TRUE;
				
					$permissions['ApplicationClient_Controller_delete']=TRUE;
				
					$permissions['ApplicationClient_Controller_get_list']=TRUE;
				
					$permissions['ApplicationClient_Controller_get_object']=TRUE;
				
					$permissions['PersonIdPaper_Controller_get_object']=TRUE;
				
					$permissions['PersonIdPaper_Controller_get_list']=TRUE;
				
					$permissions['PersonIdPaper_Controller_complete']=TRUE;
				
					$permissions['ChatMessage_Controller_insert']=TRUE;
				
					$permissions['ChatMessage_Controller_update']=TRUE;
				
					$permissions['ChatMessage_Controller_get_list']=TRUE;
				
					$permissions['ChatMessage_Controller_get_object']=TRUE;
				
					$permissions['ConstrTypeTechnicalFeature_Controller_insert']=TRUE;
				
					$permissions['ConstrTypeTechnicalFeature_Controller_update']=TRUE;
				
					$permissions['ConstrTypeTechnicalFeature_Controller_get_list']=TRUE;
				
					$permissions['ConstrTypeTechnicalFeature_Controller_get_object']=TRUE;
				
					$permissions['Holiday_Controller_insert']=TRUE;
				
					$permissions['Holiday_Controller_update']=TRUE;
				
					$permissions['Holiday_Controller_delete']=TRUE;
				
					$permissions['Holiday_Controller_get_list']=TRUE;
				
					$permissions['Holiday_Controller_get_object']=TRUE;
				
					$permissions['OutMail_Controller_insert']=TRUE;
				
					$permissions['OutMail_Controller_update']=TRUE;
				
					$permissions['OutMail_Controller_delete']=TRUE;
				
					$permissions['OutMail_Controller_get_list']=TRUE;
				
					$permissions['OutMail_Controller_get_object']=TRUE;
				
					$permissions['Department_Controller_insert']=TRUE;
				
					$permissions['Department_Controller_update']=TRUE;
				
					$permissions['Department_Controller_delete']=TRUE;
				
					$permissions['Department_Controller_get_object']=TRUE;
				
					$permissions['Department_Controller_get_list']=TRUE;
				
					$permissions['Department_Controller_complete']=TRUE;
				
					$permissions['Employee_Controller_insert']=TRUE;
				
					$permissions['Employee_Controller_update']=TRUE;
				
					$permissions['Employee_Controller_delete']=TRUE;
				
					$permissions['Employee_Controller_get_object']=TRUE;
				
					$permissions['Employee_Controller_get_list']=TRUE;
				
					$permissions['Employee_Controller_complete']=TRUE;
				
return array_key_exists($contrId.'_'.$methId,$permissions);
}
?>