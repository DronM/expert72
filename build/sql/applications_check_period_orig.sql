-- Function: public.applications_check_period(integer, timestamp with time zone, integer)

-- DROP FUNCTION public.applications_check_period(integer, timestamp with time zone, integer);

CREATE OR REPLACE FUNCTION public.applications_check_period(
...
$BODY$
	WITH
		...
		period AS (	
			SELECT d,h_from,h_to FROM w_hours
			OFFSET
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.applications_check_period(integer, timestamp with time zone, integer)
  OWNER TO expert72;
