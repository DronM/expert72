	...
	IF (TG_WHEN='AFTER' AND TG_OP='INSERT') THEN		
		...
		IF v_pay_cnt = 1 THEN
			SELECT
				...
				contracts_work_end_date(applications.office_id, t.date_type, now(), t.expert_work_day_count),
				simult_contr.id,
				CASE WHEN simult_contr.id IS NOT NULL THEN
					contracts_work_end_date(cost_eval_app.office_id, simult_contr.date_type, now(), simult_contr.expertise_day_count)
				ELSE NULL
				END,
				cost_eval_app.id,
			...
			IF coalesce(v_cost_eval_simult,FALSE)=FALSE THEN
				...
				IF v_simult_contr_id IS NOT NULL THEN
					...
					INSERT INTO application_processes
					(application_id, date_time, state, user_id, end_date_time)
					VALUES (v_simult_app_id, now(), 'expertise'::application_states, v_user_id, v_work_end_date);
