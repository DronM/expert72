	...
	ELSIF (TG_WHEN='AFTER' AND (TG_OP='INSERT' OR TG_OP='UPDATE')) THEN			
		...
		AND (NOT const_client_lk_val() OR const_debug_val()) THEN
			...
			WHERE
				...
				AND
				employees.id IN (
					SELECT (jsonb_array_elements(ct.permissions->'rows')->'fields'->'obj'->'keys'->>'id')::int
