...
<xsl:template name="extra_methods">
	...
	public function password_recover($pm){		
		try{
			...
			"SELECT id FROM users%s WHERE email=%s",
			Application_Controller::LKPostfix(),
	...
	public function register($pm){
		...
		*/
		...
		try{
			...
			$ar = $this->getDbLink()->query_first(sprintf(
				"SELECT TRUE AS ex FROM users%s WHERE name=%s OR email=%s",
				Application_Controller::LKPostfix(),
			...
			try{
				...
				"INSERT INTO users%s (role_id,name,pwd,email,name_full,pers_data_proc_agreement,time_zone_locale_id)
				...
				$ar_email_key = $this->getDbLinkMaster()->query_first(sprintf(
					"INSERT INTO user_email_confirmations%s (key,user_id)
	...
	private function field_check($pm,$field1,$field2=NULL){
		...
		return $this->getDbLink()->query_first(sprintf(
			"SELECT
				((SELECT TRUE FROM users WHERE %s) OR (SELECT TRUE FROM users_lk WHERE %s)) AS ex",
			$cond
		));
	}
