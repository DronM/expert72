...
function DocFlowInList_View(id,options){
	...
	if (!options.fromApp){
		var filters = {
			...
			,"doc_flow_type":{
				"binding":new CommandBinding({
					"control":new DocFlowTypeSelect(id+":filter-ctrl-doc_flow_type",{"type_id":"in"}),
					"field":new FieldInt("doc_flow_type_id")
				})		
			}
			,"state":{
				"binding":new CommandBinding({
					"control":new Enum_doc_flow_in_states(id+":filter-ctrl-state",{"labelCaption":"Статус:"}),
...
extend(DocFlowInList_View,ViewAjx);
