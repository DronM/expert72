 ...
 */
function Conclusion_tDocument_Container(id,options){
	options = options || {};	
	...
	options.elementOptions.addElement = function(){
		this.addElement(new Conclusion_tDocument(this.getId()+":Document",{
			"labelCaption":"Ранее подготовленное заключение"
		}));
		var self = this;
	...
	options.addElement = function(){
		this.addElement(new ButtonCmd(this.getId()+":cmdAdd",{
			"title":"Добавить документ, Документы, представленный для проведения экспертизы",
	...
	Conclusion_tDocument_Container.superclass.constructor.call(this,id,options);
}
extend(Conclusion_tDocument_Container,BaseContainerXML);//
