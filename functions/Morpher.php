...
class Morpher {
	...
	public static function morpher_inflect($cmd,&$params){
		$params['format'] = 'json';
		$par_str = '';
	...
	public static function declension($params,$dbLinkMaster=NULL,$dbLink=NULL){
		...
		if ($link_read){
			$ar = $link_read->query_first(sprintf("SELECT res FROM morpher WHERE src = '%s'",trim($params['s'])));
		}
		$res = '';
		if (!is_array($ar) || !count($ar) || !isset($ar['res'])){
			...
			if ($dbLinkMaster){
				try{
					$dbLinkMaster->query(sprintf("INSERT INTO morpher (src,res) VALUES ('%s','%s')",trim($params['s']),serialize($res)));
				}
				catch(Exception $e){			
				}
			}
